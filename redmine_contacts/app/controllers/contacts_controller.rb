...
class ContactsController < ApplicationController
  ...
  def bulk_update
    ...
    @contacts.each do |contact|
      contact.reload
      params[:contact][:tag_list] = (contact.tag_list + Redmineup::TagList.from(params[:add_tag_list]) - Redmineup::TagList.from(params[:delete_tag_list])).uniq
  ...
  def send_mails
    ...
    contacts.each do |contact|
      ...
      flash[:notice] = l(:notice_email_sent, delivered_contacts.map { |c| "#{c.name} <span class='icon icon-email'>#{view_context.sprite_icon('email')} #{c.emails.first}</span>" }.join(', ')).chomp[0, 500] if delivered_contacts.any?
