...
class RedmineContacts::TestCase
  ...
  def self.prepare
    ...
    address_cf.name = 'Test related addresses'
    address_cf.field_format = 'address'
    address_cf.is_for_all = true

    address_cf.tracker_ids = Tracker.pluck(:id)
    address_cf.save!
    addresses = Address.where(id: [3, 4])
    [[1, addresses[0]], [2, addresses[1]]].each do |issue_id, address|
      address_cf.custom_values.create!(customized: Issue.find(issue_id), address: address, value: address.to_s)
    end
  end
end
