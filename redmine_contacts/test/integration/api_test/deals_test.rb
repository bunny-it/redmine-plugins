...
class Redmine::ApiTest::DealsTest < Redmine::ApiTest::Base
  ...
  test 'GET /deals/1.json with lines' do
    deal = Deal.find(1)
    compatible_api_request :get, '/deals/1.json', { include: 'lines' }, credentials('admin')
    assert_response :success
    assert_match 'application/json', response.content_type

    json = ActiveSupport::JSON.decode(response.body)
    assert_equal json['deal']['name'], deal.name
    assert json['deal']['lines']
