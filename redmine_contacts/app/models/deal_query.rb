...
class DealQuery < Query
  ...
  self.view_permission = :view_deals

  self.available_columns = [
    QueryColumn.new(:id, :sortable => "#{Deal.table_name}.id", :caption => '#'),
  ...
  def initialize_available_filters
    add_available_filter 'ids', :type => :integer, :label => :label_deal
  ...
  def deal_amount
    @deal_amount ||= Deal.where(id: objects_scope.ids).group("#{Deal.table_name}.currency").sum(:price)
