...
class ContactQuery < Query
  ...
  def initialize_available_filters
    add_available_filter 'ids', type: :contact, label: :label_contact
    ...
    add_available_filter "is_company", :type => :list, :values => [[l(:general_text_yes), ApplicationRecord.connection.quoted_true.gsub(/'/, '')], [l(:general_text_no), ApplicationRecord.connection.quoted_false.gsub(/'/, '')]], :order => 12
  ...
  def sql_for_is_company_field(field, operator, value)
    if Rails.version >= '5.2'
      value.map! { |v| v == ApplicationRecord.connection.quoted_true.gsub(/'/, '') ? true : false }
    end
  ...
  def sql_for_has_open_issues_field(field, operator, value)
    ...
    "
    conditions = "
      (#{IssueStatus.table_name}.is_closed = #{ApplicationRecord.connection.quoted_false} OR #{IssueStatus.table_name}.is_closed IS NULL)
