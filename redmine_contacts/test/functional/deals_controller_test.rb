...
class DealsControllerTest < ActionController::TestCase
  ...
  def test_get_index_with_closed
    ...
    assert_select 'a', /First deal with contacts/
    assert_select 'table.index.excerpt td.name div a', { :count => 0, :text => /Closed deal/ }
  end
  ...
  def test_get_closed_index_with_pages
    ...
    assert_response :success
    assert_select 'h2', 'Deals'
    assert_select 'table.index.excerpt td.name div a', /Closed deal/
  end
  ...
  def test_get_index_with_filters
    ...
    assert_select 'table.index.excerpt td.name div a', /Second deal with contacts/
    assert_select 'table.index.excerpt td.name div a', { :count => 0, :text => 'Deal without contact' }
  end
  ...
  def test_get_index_without_statuses
    ...
    assert_select'span.status-badge a', :text => 'Pending(1)'
  ...
  def test_get_show
    ...
    assert_select 'div.subject_header div.name h3', %r{#{deal.name}}
  end
  ...
  def test_get_show_with_related_issue
    ...
    assert_select 'div#sidebar h3', :html => /Related issues/
  ...
  def test_get_show_with_statuses
    ...
    assert_select '.drdn-items' do
      assert_select 'a', /Pending/  # Not empty
      assert_select 'a', /Won/  # Not empty
      assert_select 'a', { :count => 0, :text => /Lost/ } # Not empty
    end
  ...
  def test_put_update
    @request.session[:user_id] = 1
    with_redmineup_settings('thousands_delimiter' => ',', 'decimal_separator' => '.') do
      deal = Deal.find(3)
      ...
      assert_response :success
      assert_select 'div.subject_info', /23\,000\.0/
  ...
  def test_should_get_index_with_cf_as_csv
    @request.session[:user_id] = 1
    ...
    compatible_request :get, :index, { :format => 'csv', "c[]" => "cf_#{field.id}", encoding: "UTF-8" }
    assert_response :success
    assert_not_nil deals_in_list
  ...
  def test_should_get_index_with_products_as_csv
    @request.session[:user_id] = 1

    products = [Product.find(1), Product.find(2)]
    deal = Deal.find(1)
    products.each do |product|
      deal_line = deal.lines.new(product: product)
      deal_line.save
    end
    c_field = DealCustomField.create!(name: 'Test custom field', is_filter: true, field_format: 'string')
    deal.custom_field_values = { c_field.id => "This is custom значение" }
    deal.save

    compatible_request :get, :index, { :format => 'csv', :c => ['all_inline'], encoding: "UTF-8" }
    assert_response :success
    assert_not_nil deals_in_list
    assert_match 'text/csv', @response.content_type
    assert_match 'Test custom field', @response.body
    assert_match 'This is custom значение', @response.body.force_encoding('utf-8')
    assert_match "#{products[0].name}, #{products[1].name}", @response.body
  ...
  def test_get_index_with_product_filter
    ...
    assert_select 'table.deals.index td.name div a', /First deal with contacts/
  end if RedmineContacts.products_plugin_installed?
