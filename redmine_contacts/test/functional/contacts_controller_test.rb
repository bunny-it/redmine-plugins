...
class ContactsControllerTest < ActionController::TestCase
  ...
  test 'should get index with filters and sorting' do
    ...
    assert_select 'div#content div#contact_list table.contacts td.name div.header a', 'Ivan Ivanov'
  end
  ...
  def test_should_get_index_with_filters
    ...
    assert_response :success
    assert_select 'div#content div#contact_list table.contacts td.name div a', full_name
  end
  ...
  def test_get_show_tab_deals
    ...
    assert_response :success
    assert_not_nil contacts_in_list
    assert_select 'div.contact.details h3', :html => /Domoway/
  ...
  def test_put_bulk_update_with_bulk_delete_tag
    @request.session[:user_id] = 1

    compatible_request :put, :bulk_update, :ids => [1, 3],
                                           :add_tag_list => '',
                                           :delete_tag_list => 'main',
                                           :add_projects_list => [],
                                           :delete_projects_list => [],
                                           :note => { :content => 'Bulk note content' },
                                           :contact => { :company => 'Bulk company', :job_title => '' }

    assert_redirected_to :controller => 'contacts', :action => 'index', :project_id => nil
    contacts = Contact.find([1, 3])
    contacts.each do |contact|
  ...
  def test_index_properly_exports_tags_as_text_in_csv
    ...
    contact = Contact.find(1)
    contact.tags = [Redmineup::Tag.new(:name => 'foo')]
    contact.save
